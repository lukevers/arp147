...
func TestCos(t *testing.T) {
	for i := 0; i < len(vf); i++ {
		if f := Cos(vf[i]); !closeenough(cos[i], f) {
...
func TestHypotGo(t *testing.T) {
	for i := 0; i < len(vf); i++ {
		...
		if f := HypotGo(1e20*tanh[i], 1e20*tanh[i]); !closeenough(a, f) {
			t.Errorf("[%d] HypotGo(%g, %g) = %g, want %g", i, 1e20*tanh[i], 1e20*tanh[i], f, a)
		}
	}
	for i := 0; i < len(vfhypotSC); i++ {
...
func TestSin(t *testing.T) {
	for i := 0; i < len(vf); i++ {
		if f := Sin(vf[i]); !closeenough(sin[i], f) {
			t.Errorf("[%d] Sin(%.14f) = %.14f, want %.14f", i, vf[i], f, sin[i])
		}
	}
...
func TestSqrt(t *testing.T) {
	for i := 0; i < len(vf); i++ {
		a := Abs(vf[i])
		if f := SqrtGo(a); sqrt_val[i] != f {
			t.Errorf("SqrtGo(%g) = %g, want %g", a, f, sqrt_val[i])
		}
		a = Abs(vf[i])
		if f := Sqrt(a); sqrt_val[i] != f {
			t.Errorf("Sqrt(%g) = %g, want %g", a, f, sqrt_val[i])
		}
	}
...
func TestSpecial(t *testing.T) {
	...
	for i, x := range vf {
		{
			s := fmt.Sprintf("%b", Float32bits(x))
			for len(s) < 32 {
				s = "0" + s
			}
			sign, exponent, mantissa := s[0:1], s[1:8], s[8:]
			t.Errorf("x: %f\tx bits: \t%s %s %s, ilogb: %d", x, sign, exponent, mantissa, Ilogb(x))
		}
		{
			y := vf64[i]
			s := fmt.Sprintf("%b", Float64bits(y))
			for len(s) < 64 {
				s = "0" + s
			}
			sign, exponent, mantissa := s[0:1], s[1:11], s[11:]
			t.Errorf("y: %f\ty bits: \t%s %s %s, ilogb: %d", y, sign, exponent, mantissa, math.Ilogb(y))
		}
		t.Errorf("")
	}
}
